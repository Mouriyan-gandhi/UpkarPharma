...
//
// Also resets each admin's password to `123456` (dev default).
...
console.log(`Found ${admins.length} admin(s). Backfilling emails…\n`);

for (const a of admins) {
  const email = `admin-${a.phone}@upkem.internal`;
  const { error: uErr } = await sb.auth.admin.updateUserById(a.id, {
    email,
    password: '123456',
    email_confirm: true,
  });
  if (uErr) {
    ...
    console.log(`  ✅ ${a.phone} (${a.store_name}) → ${email} / 123456`);
  }
}

console.log('\nAdmin login (until phone provider is enabled):');
console.log('  URL:      http://localhost:3000/login');
console.log('  Phone:    (your admin phone, e.g. 6383945610)');
